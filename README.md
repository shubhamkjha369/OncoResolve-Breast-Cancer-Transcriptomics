...
| :--- | :--- | :--- | :--- |
| **Data Hygiene (Feature Selection)** | Global feature selection/scaling applied *before* data splitting (high leakage risk). | **Strict Fold-Contained Preprocessing (ALP)**: StandardScaler, ANOVA, and LASSO are fit only inside training folds. | Prevents performance inflation and optimistic bias, guaranteeing statistical validity for clinical transfer. |
| **Cross-Platform Transfer** | Single-cohort focus (TCGA only) or direct model transfer without realignment (causes performance collapse). | **Multi-Cohort Independent Scale Realignment**: Locked models evaluated on SMC 2018 ($N=166$) and SCAN-B ($N=317$) with cohort-independent Z-scaling and alphabetical feature alignment. | Successfully bridges platform dynamic range differences (Illumina HiSeq vs NextSeq) without retraining, achieving 82%–86% accuracy. |
| **Explainability (XAI) Resolution** | Global feature importances only (e.g., Random Forest Gini or ANOVA scores). | **Personalized N-of-1 Explainability**: Combines global attributions with local patient-level LinearSHAP/KernelSHAP waterfall attributions. | Explains the exact molecular drivers behind each individual patient's PAM50 classification to support clinical trust. |
| **Outlier Profiling** | Standard outlier analysis is omitted, or restricted to simple z-score outlier detection. | **Composite Uniqueness Score (CUS)**: Integrates topological Patient Similarity Network (PSN) distances with RidgeCV reconstruction error. | Isolates patient-specific, non-consensus biological processes that are orthogonal to global PAM50 signatures (Jaccard similarity $\approx 0.0$). |
